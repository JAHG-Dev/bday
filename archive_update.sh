...
birthYear=$(date -jf "%Y-%m-%d" "$birthdate" +%Y)
birthMonth=$(date -jf "%Y-%m-%d" "$birthdate" +%m)
birthDay=$(date -jf "%Y-%m-%d" "$birthdate" +%d)
...
calculate_version() {
    ...
    diff_in_seconds=$(($(date -jf "%Y-%m-%d" "$commit_date" +%s) - $(date -jf "%Y-%m-%d" "$birthdate" +%s)))
...
while [ "$d" != "$current_date" ]; do
    ...
    diff_in_seconds=$(($(date -jf "%Y-%m-%d" "$d" +%s) - $(date -jf "%Y-%m-%d" "$birthdate" +%s)))
    ...
    # Incrementar al siguiente día (ajustado para macOS)
    d=$(date -jf "%Y-%m-%d" "$d" +"%s")
    d=$(date -r $((d + 86400)) +"%Y-%m-%d")
done
